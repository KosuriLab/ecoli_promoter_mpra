...
//			assert(kmer.equals(kmer2)) : kmer+", "+kmer2+", "+kmer.xor()+", "+kmer2.xor();
	...
	@Override
	public boolean dumpKmersAsBytes(String fname, int mincount, int maxcount, boolean printTime){
		...
		bsw.start();
		for(AbstractKmerTableU set : tables){
			set.dumpKmersAsBytes(bsw, k, mincount, maxcount);
		}
		bsw.poisonAndWait();
	...
	@Override
	public boolean dumpKmersAsBytes_MT(String fname, int mincount, int maxcount, boolean printTime){
		
		final int threads=Tools.min(Shared.threads(), tables.length);
		if(threads<3 || DumpThread.NUM_THREADS==1){return dumpKmersAsBytes(fname, mincount, maxcount, printTime);}
		...
		ByteStreamWriter bsw=new ByteStreamWriter(fname, overwrite, append, true);
		bsw.start();
		DumpThreadU.dump(k, mincount, maxcount, tables, bsw);
		bsw.poisonAndWait();
