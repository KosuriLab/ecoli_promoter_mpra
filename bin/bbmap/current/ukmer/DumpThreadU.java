...
public class DumpThreadU extends Thread{
	...
	public static boolean dump(final int k, final int mincount, final int maxcount, final AbstractKmerTableU[] tables, final ByteStreamWriter bsw){
		...
		for(int i=0; i<threads; i++){
			list.add(new DumpThreadU(k, mincount, maxcount, lock, tables, bsw));
		}
	...
	public DumpThreadU(final int k_, final int mincount_, final int maxcount_, final AtomicInteger nextTable_, final AbstractKmerTableU[] tables_, final ByteStreamWriter bsw_){
		k=k_;
		mincount=mincount_;
		maxcount=maxcount_;
	...
	public void run(){
		...
		for(int i=nextTable.getAndIncrement(); i<tables.length; i=nextTable.getAndIncrement()){
			...
			t.dumpKmersAsBytes_MT(bsw, bb, k, mincount, maxcount);
		}
		if(bb.length()>0){
