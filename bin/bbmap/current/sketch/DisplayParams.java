...
//		printColors=true;
	...
	public JsonObject toJson(Comparison c){
		...
		//Primary fields
		if(printAni){j.add((aminoOrTranslate() ? "API" : "ANI"), 100*c.ani(aniFromWkid));}
...
//				System.err.println("*"+tn.id+", "+tn.id%Colors.colorArray.length);
	...
	String header(){
		...
		if(format==FORMAT_QUERY_REF_ANI || format==FORMAT_CONSTELLATION){
			if(reportAniOnly){return "#Query\tRef\t"+(aminoOrTranslate() ? "\tAPI" : "\tANI");}
			if(format==FORMAT_QUERY_REF_ANI){return "#Query\tRef\tANI\tQSize\tRefSize\tQBases\tRBases";}
		...
		if(printAni){sb.append(aminoOrTranslate() ? "\tAPI" : "\tANI");}
