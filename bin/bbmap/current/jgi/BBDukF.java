	...
	public void process2(long startTime){
		...
		if(dump!=null){
			...
			for(AbstractKmerTable set : keySets){
				set.dumpKmersAsBytes(bsw, k, 0, Integer.MAX_VALUE);
			}
			bsw.poisonAndWait();
		}
...
//				outstream.println(r+", "+s+". Setting "+scaffoldNames.get(s)+" -> "+name);
	...
	private void spawnProcessThreads(Timer t){
		...
		final boolean paired;
		{
			...
			cris.start(); //4567
			paired=cris.paired();
			if(!ffin1.samOrBam() && !silent){outstream.println("Input is being processed as "+(paired ? "paired" : "unpaired"));}
		}
		...
		final ConcurrentReadOutputStream ros, rosb, ross;
		{
			...
			if(ros!=null || rosb!=null || ross!=null){
				t.stop();
				if(!silent){outstream.println("Started output streams:\t"+t);}
				t.start();
			}
		}
