		...
		for(Plate p : plates){
			for(Well w : p.wells){
				...
				if(kapaReads>0){
					...
					if(td==null){
						td=new TagData(w.correctKapaTag);
...
//			System.err.println("Reading "+address);
			ByteBuilder message=ServerTools.readPage(address);
...
//        "pct":0.9698492462311559
		...
		void loadKapa(JsonObject kapa){
			...
			kapaMap=new HashMap<String, KapaEntry>(3+offwells.length*2);
		...
		HashMap<String, KapaEntry> kapaMap;
...
//			double[] array=new double[list.size()];
		...
		HashMap<String, ArrayList<Double>> ppmMap=new HashMap<String, ArrayList<Double>>(203);
		HashMap<String, ArrayList<String>> plateNameMap=new HashMap<String, ArrayList<String>>(203);
	...
	private final HashMap<String, TagData> tagMap=new HashMap<String, TagData>(203);
