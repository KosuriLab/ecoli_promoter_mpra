...
inactive_predictions <- filter(predictions, active == 'inactive')
...
ggplot(predictions, aes(observed, predicted)) + 
    ...
    annotate('text', x = 75, y = 0.25, 
             label = paste0('r = ', signif(corr_inactive, 3))) +
...
corr_active <- cor.test(active_predictions$observed, active_predictions$predicted)$estimate
inactive_predictions <- filter(tss_predictions, active == 'inactive')
corr_inactive <- cor.test(inactive_predictions$observed, inactive_predictions$predicted)$estimate
...
corr_active <- cor.test(active_predictions$observed, active_predictions$predicted)$estimate
inactive_predictions <- filter(tss_split_predictions, active == 'inactive')
corr_inactive <- cor.test(inactive_predictions$observed, inactive_predictions$predicted)$estimate
...
ggplot(tss_split_predictions, aes(observed, predicted)) + 
    ...
    scale_color_manual(values = c('navyblue', 'gray38')) +
    scale_x_log10() + scale_y_log10() + annotation_logticks() +
    annotate('text', x = 25, y = 0.01, 
             label = paste0('r = ', signif(corr_inactive, 3))) +
    annotate('text', x = 25, y = 0.015, color = 'navyblue',
             label = paste0('r = ', signif(corr_active, 3))) +
