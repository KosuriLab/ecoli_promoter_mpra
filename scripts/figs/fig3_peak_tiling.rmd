...
data_all <- data_all %>% 
    mutate(category = case_when(grepl('random', .$name) ~ 'random',
                                ...
                                grepl('pos_control', .$name) ~ 'positive',
                                TRUE ~ 'tile'))
...
#neg <- subset(data_all, grepl("neg_control", data_all$name))
#neg_median <- median(neg$expn_med_fitted_scaled, na.rm = TRUE)
#neg_mad <- mad(neg$expn_med_fitted_scaled)
#threshold = neg_median+(3*neg_mad)
#threshold = .5
#5% (4.9 actually) FDR if use .5
...
ggplot(peak_data, aes(RNA_exp_sum_1, RNA_exp_sum_2)) + 
  ...
  annotate("text", label=paste("italic(r)==",signif(cor, 3)),parse=TRUE, x=2.5, y=15, size = 8) +
  ...
  annotate("text", label = "Negative Controls", x = 10, y = .5, color = 'firebrick1', size = 8) +
...
peak_data %>% 
    ...
    ggplot(aes(tile_start+peak_start, expn_med_fitted_scaled)) + geom_point(aes(color = active)) +
...
#ggsave("../../Figure3/3B_MraZPromoter.pdf")
```
...
peak_data %>% filter(expn_med_fitted_scaled > threshold) %>%
        ...
        write.table(., "../processed_data/peak_tile/active_tiles.bed", row.names = F, quote = F, sep = '\t', col.names = FALSE)
...
```{bash}
bedtools intersect -c -a ../processed_data/U00096.2_plus_minus_called_peaks_threshold1.1_merge40_min60.bed -b ../processed_data/peak_tile/merged_active_tiles.bed -s > ../processed_data/peak_tile/collapsed_per_peak.bed
...
```{r}
collapsed_per_Peak <- read.table("../processed_data/peak_tile/collapsed_per_peak.bed")#, col.names = c("genome", 'left', 'right', 'name', 'score', 'strand'))

ggplot(collapsed_per_Peak, aes(V9)) +
...
#ggsave("../../Figure3/1C_PromotersPerPeak.pdf")

```

Look at frag coverage at subpromoter peaks
```{r}
collapsed_per_Peak %>% mutate(med_size = median(V3-V2)) %>% select(med_size) %>% distinct()
#median peak size is 243.5

for (i in c(0:7)){
  tmp <- collapsed_per_Peak %>% filter(V9 == i) %>% select(V1:V6) %>% filter(V6 == '+')
  write.table(tmp, paste("../processed_data/subpromoters_plus_", i, ".bed", sep = ''), row.names = F, quote = F, col.names = FALSE, sep = '\t')
}

for (i in c(0:7)){
  tmp <- collapsed_per_Peak %>% filter(V9 == i) %>% select(V1:V6) %>% filter(V6 == '-')
  write.table(tmp, paste("../processed_data/subpromoters_minus_", i, ".bed", sep = ''), row.names = F, quote = F, col.names = FALSE, sep = '\t')
}
...
plus_subpromoter <- read.table("../processed_data/plus_subpromoter_counts.txt", skip = 3, fill = TRUE, sep = '\t', header = F)
...
minus_subpromoter <- read.table("../processed_data/minus_subpromoter_counts.txt", skip = 3, fill = TRUE, sep = '\t', header = F)
