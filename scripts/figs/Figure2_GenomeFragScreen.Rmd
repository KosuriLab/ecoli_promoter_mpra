...
plus_RNA_exp <- read.table("../../processed_data/frag/plus_gene_RNA_matrix.txt", 
                           skip = 3, fill = TRUE, sep = '\t', header = F) %>%
    select(V1:V145)
...
minus_RNA_exp <- read.csv("../../processed_data/frag/minus_gene_RNA_matrix.txt", 
                          skip =3, sep = '\t', header = F) %>%
    select(V146:V290)
...
```{r}
positions <- seq(10,1450,10)
plus_prom_exp <- read.table("../../processed_data/frag/plus_gene_promoter_matrix.txt", skip = 3, fill = TRUE, sep = '\t', header = F) %>%
    select(V1:V145) 
...
minus_prom_exp <- read.csv("../../processed_data/frag/minus_gene_promoter_matrix.txt", skip =3, sep = '\t', header = F) %>%
    select(V146:V290) 
