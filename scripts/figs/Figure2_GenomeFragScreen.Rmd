...
plus_tss <- read.table("../../processed_data/frag_peak_calling/lb/plus_tss_counts.txt", 
...
minus_tss <- read.table("../../processed_data/frag_peak_calling/lb/minus_tss_counts.txt", 
                        skip =3, sep = '\t', header = F)
...
bedtools intersect -v -s -a ../../processed_data/frag_peak_calling/lb/U00096.2_plus_minus_called_peaks_threshold1.1_merge40_min60.bed -b ../../processed_data/frag_peak_calling/m9/U00096.2_M9_plus_minus_called_peaks_threshold1.1_merge40_min60.bed > ../../processed_data/frag_peak_calling/lb/Unshared_LB_peaks.bed

bedtools intersect -v -s -a ../../processed_data/frag_peak_calling/m9/U00096.2_M9_plus_minus_called_peaks_threshold1.1_merge40_min60.bed -b ../../processed_data/frag_peak_calling/lb/U00096.2_plus_minus_called_peaks_threshold1.1_merge40_min60.bed > ../../processed_data/frag_peak_calling/m9//Unshared_M9_peaks.bed
```

```{r}
M9_num <- read.table("../../processed_data/frag_peak_calling/m9/Unshared_M9_peaks.bed") %>% nrow()

LB_num <- read.table("../../processed_data/frag_peak_calling/lb/Unshared_LB_peaks.bed") %>% nrow()

M9_total <- read.table("../../processed_data/frag_peak_calling/m9/U00096.2_M9_plus_minus_called_peaks_threshold1.1_merge40_min60.bed") %>% nrow()
...
promoter_max_LB <- read.table("../../processed_data/frag_peak_calling/lb/U00096.2_plus_minus_called_peaks_threshold1.1_merge40_min60.bed", header = F) %>%
...
promoter_max_M9 <- read.table("../../processed_data/frag_peak_calling/m9/U00096.2_M9_plus_minus_called_peaks_threshold1.1_merge40_min60.bed", header = F) %>%
    select(peak_name = V4, peak_max = V8, peak_score = V7, peak_strand =  V6) %>%
...
plus_RNA_exp <- read.table("../../processed_data/frag_peak_calling/plus_gene_RNA_matrix.txt", 
                           skip = 3, fill = TRUE, sep = '\t', header = F) %>%
    select(V1:V145)
plus_RNA_exp[is.nan(plus_RNA_exp)] <- 0
names(plus_RNA_exp) <- positions

plus_sense_RNA <-plus_RNA_exp[c(1:721),]
plus_dual_RNA <-plus_RNA_exp[c(722:861),]
plus_anti_RNA <- plus_RNA_exp[c(862:1314),]

minus_RNA_exp <- read.csv("../../processed_data/frag_peak_calling/minus_gene_RNA_matrix.txt", 
                          skip =3, sep = '\t', header = F) %>%
    select(V146:V290)
minus_RNA_exp[is.nan(minus_RNA_exp)] <- 0

names(minus_RNA_exp) <- positions
minus_sense_RNA <-minus_RNA_exp[c(1:703),]
minus_dual_RNA <-minus_RNA_exp[c(704:835),]
minus_anti_RNA <- minus_RNA_exp[c(835:1286),]
...
```{r}
positions <- seq(10,1450,10)
plus_prom_exp <- read.table("../../processed_data/frag_peak_calling/plus_gene_promoter_matrix.txt", skip = 3, fill = TRUE, sep = '\t', header = F) %>%
    select(V1:V145) 
names(plus_prom_exp) <- positions

plus_sense_prom <-plus_prom_exp[c(1:721),]
plus_dual_prom <-plus_prom_exp[c(722:861),]
plus_anti_prom <- plus_prom_exp[c(862:1314),]

minus_prom_exp <- read.csv("../../processed_data/frag_peak_calling/minus_gene_promoter_matrix.txt", skip =3, sep = '\t', header = F) %>%
    select(V146:V290) 
names(minus_prom_exp) <- positions
minus_sense_prom <-minus_prom_exp[c(1:703),]
minus_dual_prom <-minus_prom_exp[c(704:835),]
minus_anti_prom <- minus_prom_exp[c(835:1286),]


prom_mean_sense <- bind_rows(plus_sense_prom, minus_sense_prom) %>%
    as.matrix() %>% 
...
prom_mean_dual <- bind_rows(plus_dual_prom, minus_dual_prom) %>%
    as.matrix() %>% 
    colMedians(na.rm = TRUE)  %>%
    rbind(positions, .) %>% 
    t() %>%
...
prom_mean_anti <- bind_rows(plus_anti_prom, minus_anti_prom) %>%
    as.matrix() %>% 
    colMedians(na.rm = TRUE) %>%
    rbind(positions, .) %>% t() %>%
    as.data.frame() %>% 
...
b <- bind_rows(prom_mean_sense, prom_mean_anti, prom_mean_dual) %>% 
    ggplot(., aes(x=position, y=coverage)) +
    geom_area(alpha = 0, size = 1.1, position = 'identity', aes(color = Type)) + 
    geom_vline(xintercept = c(300,1150), linetype = 'dashed') +
...
ggsave('../../figs/fig2/antisense_gene.pdf', units = 'in', width = 16, height = 5)
```
