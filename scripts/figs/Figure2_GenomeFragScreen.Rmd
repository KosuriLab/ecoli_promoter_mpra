...
plus_RNA_exp <- read.table("../processed_data/plus_transcriptome_counts.txt", skip = 3, fill = TRUE, sep = '\t', header = F) %>%
    select(V1:V145)
...
minus_RNA_exp <- read.csv("../processed_data/minus_transcriptome_counts.txt", skip =3, sep = '\t', header = F) %>%
    select(V1:145)
...
RNA_mean_sense <- rbind(plus_sense_RNA, minus_sense_RNA) %>%
                      ...
                      rbind(positions, .) %>% t() %>% as.data.frame() %>% mutate(type = 'Sense Regulated')
...
a <- rbind(RNA_mean_sense, RNA_mean_anti, RNA_mean_dual) %>% ggplot(., aes(x=position, y=coverage)) +
...
```{r}
positions <- seq(10,1450,10)
plus_prom_exp <- read.table("../processed_data/plus_promoter_M9_counts.txt", skip = 3, fill = TRUE, sep = '\t', header = F) %>%
    select(V1:V145) 
...
minus_prom_exp <- read.csv("../processed_data/minus_promoter_M9_counts.txt", skip =3, sep = '\t', header = F) %>%
    select(V1:V145) 
...
b <- rbind(prom_mean_sense, prom_mean_anti, prom_mean_dual) %>% ggplot(., aes(x=position, y=coverage)) +
          geom_area(alpha = 0, size = 1.1, position = 'identity', aes(color = Type)) + 
          geom_vline(xintercept = c(300,1150), linetype = 'dashed') +
         scale_color_manual(breaks = c('Sense Regulated \n (N=1,424)', 'Antisense Regulated \n (N=905)', "Dual Regulated \n (N=272)"),
                             values = c('firebrick3','purple', '#2E6EB7')) +
          labs(x ='Position Relative to Gene',
