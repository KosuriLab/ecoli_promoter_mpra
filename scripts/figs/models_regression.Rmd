...
```{r}
kmer_predictions <- read.table('../../processed_data/combined/20190610_kmer_linear_predictions.txt',
...
```{r}
kmer_pls_predictions <- read.table('../../processed_data/combined/20190610_kmer_pls_predictions.txt',
                               col.names = c('predicted', 'observed'))
...
```{r}
rfr_onehot <- read.table('../../processed_data/combined/20190611_rfr_onehot_tss_scramble_peak_floored_predictions.txt',
                                     col.names = c('predicted', 'observed'))
...
```{r}
rfr <- read.table('../../processed_data/combined/20190617_rfr_tss_scramble_peak_floored_predictions.txt',
                              col.names = c('predicted', 'observed'))
...
ggplot(rfr, aes(predicted, observed)) + geom_point() +
    geom_smooth(method = 'lm') + 
    annotate('text', x = 20, y = 1, parse = T, label = paste('R^2==', signif(r.squared_rfr, 3))) +
    scale_x_log10() + scale_y_log10() + annotation_logticks() +
    labs(title = 'Random forest regression (k-mer counts)')
```

```{r}
mlp <- read.table('../../processed_data/combined/20190610_mlp_tss_scramble_peak_predictions.txt',
                  col.names = c('predicted', 'observed'))
...
```{r}
nn <- read.table('../../processed_data/combined/20190611_tss_scramble_peak_regression_hyperparam_tuned_predictions.txt',
