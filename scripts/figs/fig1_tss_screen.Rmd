...
data.frame(Expression = c("Active TSS", "Inactive TSS"),
                         Value = c(nrow(filter(Wanner, expn_med > threshold)),
                                   nrow(filter(Wanner, expn_med < threshold))),
                         source = "Wanner") %>%
  rbind(., data.frame(Expression = c("Active TSS", "Inactive TSS"),
                         Value = c(nrow(filter(Storz, expn_med > threshold)),
                                   nrow(filter(Storz, expn_med < threshold))),
                         source = "Storz")) %>%
 rbind(., data.frame(Expression = c("Active TSS", "Inactive TSS"),
                         Value = c(nrow(filter(regulondb, expn_med > threshold)),
                                   nrow(filter(regulondb, expn_med < threshold))),
                         source = "RegulonDB")) %>%
...
Endo2_stats$minus10_matches <- sapply(as.character(Endo2_stats$variant),function(x) matchPWM(pwm_s70,str_sub(x,-150,-1)))
Endo2_stats$minus10_scores <- sapply(Endo2_stats$minus10_matches,function(x) PWMscoreStartingAt(pwm_s70, subject(x),start(x)))
