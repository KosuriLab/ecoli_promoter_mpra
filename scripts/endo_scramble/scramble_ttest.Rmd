...
scramble_merged <- merged_overlap %>% 
    group_by(scrambles, site_type) %>% 
    summarise(num_tfbs = sum(!is.na(tf_name))) %>% 
    select(-site_type) %>% 
    right_join(scramble_merged, by = 'scrambles') %>% 
    mutate(site_length = scramble_end_pos - scramble_start_pos) %>% 
    ungroup()
```

```{r}
scramble_merged <- scramble_merged %>% 
    left_join(select(merged_overlap, -scramble_start_pos, -scramble_end_pos, -site_type), by = 'scrambles') %>% 
    left_join(select(tf_sites, tfbs_left, tfbs_right, tf_name, tf_effect = expression_effect) %>% distinct(),
...
scramble_merged_parts <- scramble_merged %>% 
    ...
    left_join(select(data, name, tss_name, relative_exp, unscrambled_exp, mean_diff),
...
merged_tf_concordance <- function(scramble_name, tf_effect, scramble_merged_parts) {
    ...
    if(is.na(tf_effect)) {
        ...
        concordance <- any(scramble_parts$mean_diff > 0)
    } 
    else if (tf_effect == '+') {
        concordance <- any(scramble_parts$mean_diff < 0)
    }
...
scramble_merged <- scramble_merged %>% 
    distinct(scrambles, tfbs_left, tfbs_right, tf_name, tf_effect) %>% 
    rowwise() %>% 
    mutate(tf_concordance = merged_tf_concordance(scrambles, tf_effect, scramble_merged_parts)) %>% 
    ungroup()
